...
cp -R /tmp/* ../mnt2/TMP
timestamp=$(date +"%Y-%m-%d %T")
echo "/tmp Temporary file copy Timestamp : $timestamp" >> ../mnt2/Forensic_Info.txt

# Collect all files in the /var/tmp directory
echo "Collecting temporary files in /var/tmp directory..."
cp -R /var/tmp/* ../mnt2/TMP
timestamp=$(date +"%Y-%m-%d %T")
echo "/var/tmp Temporary file copy Timestamp : $timestamp" >> ../mnt2/Forensic_Info.txt

# Collect all files in the /dev/shm directory
echo "Collecting temporary files in /dev/shm directory..."
cp -R /dev/shm* ../mnt2/TMP
timestamp=$(date +"%Y-%m-%d %T")
echo "/dev/shm Temporary file copy Timestamp : $timestamp" >> ../mnt2/Forensic_Info.txt
